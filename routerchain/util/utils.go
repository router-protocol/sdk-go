...
const (
	// MainnetChainID defines the Router EIP155 chain ID for mainnet
	MainnetChainID = "router_9605"
	// TestnetChainID defines the Router EIP155 chain ID for testnet
	TestnetChainID = "router_9605"
	// BaseDenom defines the Router mainnet denomination
	BaseDenom = "route"
)

// IsMainnet returns true if the chain-id has the Router mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Router testnet EIP155 chain prefix.
...
// GetRouterAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `router`).
